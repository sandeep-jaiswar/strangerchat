            ...
            <div key={index} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[80%] rounded-2xl px-5 py-3 ${
                  isMe ? "rounded-tr-sm bg-indigo-600 text-white" : "rounded-tl-sm bg-slate-800 text-slate-100"
                }`}
              >
      ...
      <div className="border-t border-slate-800 bg-slate-900 p-4">
        <form onSubmit={handleSend} className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-white placeholder-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none"
          />
          <button
            type="submit"
            disabled={!input.trim()}
